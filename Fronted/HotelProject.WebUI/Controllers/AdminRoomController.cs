...
namespace HotelProject.WebUI.Controllers
{
    public class AdminRoomController : Controller
    {
        ...
        public async Task<IActionResult> AddRoom(CreateRoomDto createRoomDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5155/api/Room", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5155/api/Room/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5155/api/Room/{id}");
        ...
        public async Task<IActionResult> UpdateRoom(UpdateRoomDto updateRoomDto)
        {
            ...
            var responseMessage = await client.PutAsync("http://localhost:5155/api/Room/", stringContent);
